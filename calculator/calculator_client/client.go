...
func doErrorUnary(c calculatorpb.CalculatorServiceClient) {
	...
	squareRoot(c, int32(10))
	squareRoot(c, int32(-2))
}

func squareRoot(c calculatorpb.CalculatorServiceClient, number int32) {
	req := &calculatorpb.FindSquareRootRequest{
		Number: number,
	}

	res, err := c.FindSquareRoot(context.Background(), req)
	if err != nil {
	...
	fmt.Printf("Result from square root of %v: %v\n", number, res.GetResult())
}
